...
class AdminController extends Controller
{
    ...
    public function orderDetail(){
        ...
        if(Auth::guard('admin')->user()->store_id == null){
            ...
            $orderDetailscustomer = Order::select('orders.*','customer.name as name')->where(['company_id' => $reCompanyId])->where('check_deliveryDate',Carbon::now()->toDateString())->join('customer','orders.user_id','=','customer.id');
            ...
            $orderDetailscustomer = Order::select('orders.*','customer.name as name')->where(['store_id' => $reCompanyId])->where('check_deliveryDate',Carbon::now()->toDateString())->join('customer','orders.user_id','=','customer.id');
