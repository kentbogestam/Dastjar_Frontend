...
//                  getDeviceToken();
	...
	<style>		
		...
		#popup-list-discount .ui-grid-a {
			border-bottom: 1px solid #ddd;
		}

		#popup-list-discount .ui-block-a {
			width: 80%
		}

		#popup-list-discount .ui-block-b {
			width: 20%
		}

		/* block-promotion */
		.block-promotion .ui-controlgroup, .block-promotion .ui-controlgroup-controls {
			width: 100%;
		}
		.block-promotion .ui-controlgroup-controls > .ui-select {
			width: 30%;
		}
		.block-promotion .ui-controlgroup-controls .ui-first-child {
			font-size: 13px !important;
		}
		.block-promotion .ui-controlgroup-controls > .ui-last-child {
			width: 70%;
		}
		.block-promotion .ui-controlgroup-controls .ui-input-text {
			padding: 0px;
				...
				<div class="ui-grid-a">
								...
								@foreach($menuDetails as $productDetail)
									@foreach($productDetail->storeProduct as $menuDetail)
										...
										<ul data-role="listview" data-inset="true">
											<li>
												<img class="product_image" src="{{$menuDetail->small_image}}" onerror="this.src='{{ asset('images/placeholder-image.png') }}'">
					...
					<div class="ui-block-b second-part">
						<div class="seprate_qr">
							...
							<div class="">
								<img src="https://chart.googleapis.com/chart?chs=100x100&cht=qr&chl=https://anar.dastjar.com/">
							</div>
							...
							<div class="ui-grid-solo block-promotion">
								<div class="ui-block-a">
									<div data-role="controlgroup" data-type="horizontal">
										<select name="phone_prefix" id="phone_prefix">
											@php
											echo getListPhonePrifix();
											@endphp
										</select>
										<input type="number" name="number-1" id="number-1" placeholder="Enter mobile number here" data-wrapper-class="controlgroup-textinput ui-btn">
									</div>
								</div>
							</div>
							...
							<div class="ui-grid-a">
								<div class="ui-block-a">
									<button type="button" class="ui-shadow ui-btn ui-corner-all ui-mini btn-get-app" onclick="sendPromotionalApp(this)">{{ __('messages.getTheApp') }}</button>
								</div>
								<div class="ui-block-b"><input type="button" value="{{ __('messages.getDiscount') }}" data-mini="true" class="btn-show-discount-list"></div>
							</div>
						</div>
					</div>
				</div>
				...
				@foreach($discount as $row)
					<div class="ui-grid-a">
						...
						<div class="ui-block-b">
							<span class="btn-send-discount" onclick="sendPromotionalDiscount(this)" data-code="{{ $row->code }}">Send</span>
						</div>
					</div>
				@endforeach
			@else
...
@section('footer-script')
	...
	<script type="text/javascript">
		...
		// 
		$('#opt_delivery_type').change(function() {
			$('input[name=delivery_type]').val($(this).val());
		});
		...
		// Show discount list popup
		$('.btn-show-discount-list').click(function() {
			var number = $('#number-1').val();
			
			if(number.length == 9 || number.length == 10)
			{
				$('#popup-list-discount').dialog();
			}
			else
			{
		...
		// Send promotional discount to user
		function sendPromotionalDiscount(th)
		{
			var $this = $(th);
			var mobile = $('#number-1').val();
			var discount_code = $this.data('code');
			var prefix = $('#phone_prefix').val();

			// 
			if((mobile.length == 9 || mobile.length == 10) && discount_code.length && prefix.length)
			{
				$this.text('Sending...');
				...
				$.ajax({
					type: 'POST',
					url: "{{ url('kitchen/send-promotional-discount') }}",
					data: {
					...
					success: function(response) {
						...
						{
							$this.text('Sent!');
							$this.removeAttr('onclick').removeAttr('class');
						}
						else
						{
			...
			{
				$('#popup-list-discount').dialog('close');
			}
		}

		// Get the app
		function sendPromotionalApp(th)
		{
			var mobile = $('#number-1').val();
			var prefix = $('#phone_prefix').val();

			// 
			if((mobile.length == 9 || mobile.length == 10) && prefix.length)
			{
				$.ajax({
					type: 'POST',
					url: "{{ url('kitchen/send-promotional-app') }}",
					data: {
						'_token'		: $('meta[name=_token]').attr('content'),
						'prefix'		: prefix,
						'mobile'		: mobile
